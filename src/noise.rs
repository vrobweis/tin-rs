...
fn grad_xyz( hash: usize, x: Double, y: Double, z:Double ) -> Double {
    ...
    let u: Double = {                           // INTO 12 GRADIENT DIRECTIONS.
        if h < 8 { x } else {y}
    };

    let v: Double = {
        if h < 4 { y } else if (h == 12)||(h==14) {x} else {z}
    };
...
pub fn noise_xyz( x: Double, y: Double, z: Double ) -> Double {
    ...
    return lerp( w, lerp( v, lerp( u, grad_xyz( P[AA], x, y, z),           // AND ADD
                                                  grad_xyz( P[BA], x-1., y, z)),        // BLENDED
                                    lerp( u, grad_xyz( P[AB], x, y-1., z),         // RESULTS
                                                  grad_xyz( P[BB], x-1., y-1., z))),     // FROM 8
                      lerp(v, lerp( u, grad_xyz( P[AA+1], x, y, z-1.),       // CORNERS
                                                  grad_xyz( P[BA+1], x-1., y, z-1.)),    // OF CUBE
                                    lerp( u, grad_xyz( P[AB+1], x, y-1., z-1.),
